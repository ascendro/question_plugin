class QuestionMailer < Mailer
  ...
  def asked_question(journal)
    ...
    subject "[Frage zu #{question.issue.project.name} ##{question.issue.id}] #{question.issue.subject}"
    recipients question.assigned_to.mail unless question.assigned_to.nil?
    @from  = "#{question.author.name} (Serviceleitstelle) <#{Setting.mail_from}>" unless question.author.nil?
  ...
  def answered_question(question, closing_journal)
    subject "[Antwort zu  #{question.issue.project.name} ##{question.issue.id}] #{question.issue.subject}"
